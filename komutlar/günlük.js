...
exports.run = async (client, message, args) => {
  ...
  if (times !== null && day - (Date.now() - times) > 0) {
    ...
    message.channel.send({ content: `Bu gün **${ödül} Beta Coin** kazandın! Bu komutu ${time.hours ? time.hours + "saat" : ""} ${time.minutes ? time.minutes + "dakika" : ""} ${time.seconds ? time.seconds + "saniye**": ""} **2. kez yazdığınızda** ne zaman kullanabilceğini görebilirsiniz.`, reply: { messageReference: message.id }});
