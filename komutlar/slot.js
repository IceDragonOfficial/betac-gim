...
exports.run = async (client, message, args) => {
  ...
  if (bahis > 50000) return message.channel.send({ content: "En fazla 50.000 Beta Coin bahis oynayabilirsin.", reply: { messageReference: message.id } });
  if (bahis === 0) return message.channel.send({ content: "En az 1 Beta Coin bahis oynamalısın.", reply: { messageReference: message.id } });
  ...
  if(slot1 === slot2 && slot1 === slot3) {
    if(slot1 === "cvd") {
      ...
      **Slot 1:** ${slot1}, **Slot 2:** ${slot2}, **Slot3:** ${slot3}
      :tada: Yaşasın oynadığın bahsin **3 katını kazandın**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
    }
    if(slot1 === "muz") {
      ...
      return message.channel.send({ content: `
      **Slot 1:** ${slot1}, **Slot 2:** ${slot2}, **Slot3:** ${slot3}
      :tada: Yaşasın oynadığın bahsin **2 katını kazandın**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
    }
    if(slot1 === "patlıcan") {
      db.yaz(`slotsüresi_${message.author.id}`, Date.now());
      return message.channel.send({ content: `
      **Slot 1:** ${slot1}, **Slot 2:** ${slot2}, **Slot3:** ${slot3}
      :tada: Ne kazandın ne kaybettin **berabere** paran geri verildi! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
    }
  } else {
    db.çıkar(`para_${message.author.id}`, parseInt(bahis))
    return message.channel.send({ content: `
    **Slot 1:** ${slot1}, **Slot 2:** ${slot2}, **Slot3:** ${slot3}
    :cry: Olamaz bahisi **kaybettin**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
  }
}
exports.conf = {
