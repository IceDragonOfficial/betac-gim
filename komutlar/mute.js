exports.run = async (client, message, args) => {
  ...
  await request.get(`https://discord.com/api/v8/guilds/${message.guild.id}/members/${member.user.id}`, headers, async function(err, data, status, headers) {
    ...
    if(!args[1] || isNaN(args[1]) || !args[2] || !['saniye', 'dakika', 'saat', 'gün', 'hafta', 'yıl'].includes(args[2])) return message.reply({ content: 'Lütfen bir süre belirtin. örnek: .mute @etiket 10 saniye' });
