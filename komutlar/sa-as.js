...
exports.run = async (bot, message, args) => {
    ...
    if (!message.member.permissions.has('MANAGE_GUILD')) return message.channel.send(':warn:Bu komudu kullanabilmek için `Sunucuyu Yönet` yetkisine sahip olmalısın!')
    if (!args[0]) return message.channel.send(':warn:**Sa-as yazısını açmak için `.sa-as aç veya kapat`**')
