...
exports.run = async (client, message, args) => {
  ...
  if (Covid === "kazandın") {
    ...
    message.channel.send({ content: `:tada: Yaşasın bahisi **kazandın**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
  }
  if (Covid === "kaybettin") {
    db.çıkar(`para_${message.author.id}`, parseInt(bahis))
    message.channel.send({ content: `:cry: Olamaz bahisi **kaybettin**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
  }
